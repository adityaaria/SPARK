...
mkdir -p "$legacy_home/.config/spark/skills"
assert_command_output \
    "SessionStart omits obsolete legacy custom-skill warning" \
    "nested" \
    "" \
    "SPARK now uses"$'\037'"~/.config/spark/skills"$'\037'"~/.claude/skills"$'\037'"legacy" \
    "$legacy_home" \
    CLAUDE_PLUGIN_ROOT="$REPO_ROOT" \
    bash "$HOOK_UNDER_TEST"
...
mkdir -p "$codex_legacy_home/.config/spark/skills" "$codex_legacy_data"
assert_command_output \
