...
#   $OPENCODE_CONFIG_DIR/spark/             ← package root
#   $OPENCODE_CONFIG_DIR/spark/skills/      ← skills dir (../../skills from plugin)
#   $OPENCODE_CONFIG_DIR/spark/.opencode/plugins/spark.js ← plugin file
#   $OPENCODE_CONFIG_DIR/plugins/spark.js   ← symlink OpenCode reads

SPARK_DIR="$OPENCODE_CONFIG_DIR/spark"
SPARK_SKILLS_DIR="$SPARK_DIR/skills"
SPARK_PLUGIN_FILE="$SPARK_DIR/.opencode/plugins/spark.js"

# Install skills
mkdir -p "$SPARK_DIR"
cp -r "$REPO_ROOT/skills" "$SPARK_DIR/"

# Install plugin
mkdir -p "$(dirname "$SPARK_PLUGIN_FILE")"
cp "$REPO_ROOT/.opencode/plugins/spark.js" "$SPARK_PLUGIN_FILE"

# Register plugin via symlink (what OpenCode actually reads)
mkdir -p "$OPENCODE_CONFIG_DIR/plugins"
ln -sf "$SPARK_PLUGIN_FILE" "$OPENCODE_CONFIG_DIR/plugins/spark.js"
...
echo "Skills dir:           $SPARK_SKILLS_DIR"
echo "Plugin file:          $SPARK_PLUGIN_FILE"
echo "Plugin registered at: $OPENCODE_CONFIG_DIR/plugins/spark.js"
...
export REPO_ROOT
export SPARK_DIR
export SPARK_SKILLS_DIR
export SPARK_PLUGIN_FILE
