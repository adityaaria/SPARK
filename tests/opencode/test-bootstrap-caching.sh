...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SPARK_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$SPARK_SKILLS_DIR/using-spark/SKILL.md" "$TEST_HOME/using-spark.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SPARK_PLUGIN_FILE" missing
}
