#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the spark plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/spark.js"
...
skill_count=$(find "$SPARK_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $SPARK_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-spark skill exists (critical for bootstrap)
echo "Test 3: Checking using-spark skill (required for bootstrap)..."
if [ -f "$SPARK_SKILLS_DIR/using-spark/SKILL.md" ]; then
    echo "  [PASS] using-spark skill exists"
else
    echo "  [FAIL] using-spark skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$SPARK_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/spark/' "$SPARK_PLUGIN_FILE"; then
