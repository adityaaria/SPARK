...
# 1. Create in spark location (lowest priority)
mkdir -p "$SPARK_SKILLS_DIR/priority-test"
cat > "$SPARK_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: SPARK version of priority test skill
---
# Priority Test Skill (SPARK Version)

This is the SPARK version of the priority test skill.

PRIORITY_MARKER_SPARK_VERSION
EOF
...
if [ -f "$SPARK_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] SPARK version exists"
else
    echo "  [FAIL] SPARK version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled spark priority
echo ""
echo "Test 2: Documenting personal vs spark priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled spark version instead of personal version"

# Test 3: Document project vs bundled spark priority
echo ""
echo "Test 3: Documenting project vs personal/spark priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled spark version instead of project version"

# Test 4: Test a non-colliding bundled spark skill is still available
echo ""
echo "Test 4: Testing non-colliding spark skill remains available..."

mkdir -p "$SPARK_SKILLS_DIR/spark-only-test"
cat > "$SPARK_SKILLS_DIR/spark-only-test/SKILL.md" <<'EOF'
---
name: spark-only-test
description: SPARK-only priority test skill
---
# SPARK Only Test Skill

PRIORITY_MARKER_SPARK_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"spark-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_SPARK_ONLY_VERSION" "Non-colliding spark skill is still registered"
