...
main() {
    ...
    if [[ "$dir" == "$repo/.spark/sdd" ]]; then
        pass "prints <repo-root>/.spark/sdd"
    else
        fail "prints <repo-root>/.spark/sdd"
        echo "    got: $dir"
    fi

    if [[ -f "$repo/.spark/sdd/.gitignore" && "$(cat "$repo/.spark/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created with '*'"
    else
        fail "self-ignoring .gitignore created with '*'"
    fi

    printf 'x\n' > "$repo/.spark/sdd/artifact.md"
    local status
...
PLAN
    ...
    case "$brief_path" in
        "$repo/.spark/sdd/"*) pass "task-brief writes its brief under the workspace" ;;
        *)
    ...
    case "$rp_path" in
        "$repo/.spark/sdd/"*) pass "review-package writes its diff under the workspace" ;;
        *)
    ...
    if [[ "$wt_dir" == "$wt_root/.spark/sdd" && "$wt_dir" != "$dir" ]]; then
