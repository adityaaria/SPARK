...
EOF
    ...
    cat > "$repo/assets/spark-small.svg" <<'EOF'
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/spark/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/spark/.fixture-keep"
    cat > "$repo/plugins/spark/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/spark/.fixture-keep
    git -C "$repo" add plugins/spark/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/spark/skills/example/agents"

    cat > "$repo/plugins/spark/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/spark/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/spark/.codex-plugin" \
        "$repo/plugins/spark/.private-journal" \
        "$repo/plugins/spark/assets" \
        "$repo/plugins/spark/hooks" \
        "$repo/plugins/spark/skills/example/agents" \
        "$repo/plugins/spark/skills/example"

    cat > "$repo/plugins/spark/.codex-plugin/plugin.json" <<EOF
{
  "name": "spark",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/spark/assets/spark-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/spark/assets/app-icon.png"

    cat > "$repo/plugins/spark/hooks/hooks-codex.json" <<'EOF'
{
  "hooks": {
    "SessionStart": [
      {
...
EOF
    ...
    cat > "$repo/plugins/spark/hooks/session-start" <<'EOF'
#!/usr/bin/env sh
echo "session-start fixture"
EOF
    cat > "$repo/plugins/spark/hooks/session-start-codex" <<'EOF'
#!/usr/bin/env sh
echo "session-start-codex fixture"
EOF

    cat > "$repo/plugins/spark/hooks/run-hook.cmd" <<'EOF'
@echo off
echo run-hook fixture
EOF
    chmod +x "$repo/plugins/spark/hooks/session-start" "$repo/plugins/spark/hooks/session-start-codex" "$repo/plugins/spark/hooks/run-hook.cmd"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/spark/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/spark/.codex-plugin/plugin.json \
        plugins/spark/assets/app-icon.png \
        plugins/spark/assets/spark-small.svg \
        plugins/spark/hooks/hooks-codex.json \
        plugins/spark/hooks/run-hook.cmd \
        plugins/spark/hooks/session-start \
        plugins/spark/hooks/session-start-codex \
        plugins/spark/skills/example/agents/openai.yaml \
        plugins/spark/skills/example/SKILL.md \
        plugins/spark/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/spark/.kimi-plugin" \
        "$repo/plugins/spark/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/spark/.fixture-keep"
    printf '{"name":"stale-kimi"}\n' > "$repo/plugins/spark/.kimi-plugin/plugin.json"
    printf 'stale ignored leak\n' > "$repo/plugins/spark/.private-journal/leak.txt"
    git -C "$repo" add \
        plugins/spark/.fixture-keep \
        plugins/spark/.kimi-plugin/plugin.json
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/spark/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/spark/skills/example/agents/openai.yaml"
    ...
    assert_contains "$preview_section" "assets/spark-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (spark-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/spark-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/spark/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/spark" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/spark-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/spark'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/spark-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/spark-*" "Clean no-op local apply does not create sync branch in destination checkout"
