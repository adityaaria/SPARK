...
const extensionPath = resolve(repoRoot, '.pi/extensions/spark.ts');
const piToolsPath = resolve(repoRoot, 'skills/using-spark/references/pi-tools.md');
...
test('startup context injects the bootstrap as one user message until agent_end', async () => {
  ...
  assert.match(textOf(result.messages[0]), /You have spark/);
  ...
  assert.match(textOf(repeatedProviderRequest.messages[0]), /You have spark/);
...
test('session_compact injects bootstrap after compaction summaries, not before compaction', async () => {
  ...
  assert.match(textOf(result.messages[1]), /You have spark/);
