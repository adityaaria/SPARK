...
**Goal:** Add full spark support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/spark-codex` (lines 40-74)
...
- Reference: `.codex/spark-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'spark' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/spark-codex` (lines 212-280)
 ...
 * (personal skills override spark skills).
 *
 * @param {string} skillName - Name like "spark:brainstorming" or "my-skill"
 * @param {string} sparkDir - Path to spark skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, sparkDir, personalDir) {
    // Strip spark: prefix if present
    const forceSPARK = skillName.startsWith('spark:');
    const actualSkillName = forceSPARK ? skillName.replace(/^spark:/, '') : skillName;

    // Try personal skills first (unless explicitly spark:)
    if (!forceSPARK && personalDir) {
    ...
    // Try spark skills
    if (sparkDir) {
        const sparkPath = path.join(sparkDir, actualSkillName);
        const sparkSkillFile = path.join(sparkPath, 'SKILL.md');
        if (fs.existsSync(sparkSkillFile)) {
            return {
                skillFile: sparkSkillFile,
                sourceType: 'spark',
                skillPath: actualSkillName
            };
        }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/spark-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/spark-codex` (add import at top)
...
Run: `node -c .codex/spark-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/spark-codex
...
**Files:**
- Modify: `.codex/spark-codex` (lines 40-74)
...
Run: `.codex/spark-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/spark-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/spark-codex` (lines 16-38, approximately)
...
Run: `.codex/spark-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/spark.js`
...
const sparkSkillsDir = path.join(homeDir, '.config/opencode/spark/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const SPARKPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/spark.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/spark.js
...
**Files:**
- Modify: `.opencode/plugin/spark.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "spark:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > spark)
...
Run: `node -c .opencode/plugin/spark.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the spark and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const sparkSkills = skillsCore.findSkillsInDir(
            sparkSkillsDir,
            'spark',
            3
          );
          ...
          const allSkills = [...personalSkills, ...sparkSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install spark skills to ~/.config/opencode/spark/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'spark:';
...
```javascript
    'session.started': async () => {
      // Read using-spark skill content
      const usingSPARKPath = skillsCore.resolveSkillPath(
        'using-spark',
        sparkSkillsDir,
        personalSkillsDir
      );

      let usingSPARKContent = '';
      if (usingSPARKPath) {
        const fullContent = fs.readFileSync(usingSPARKPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingSPARKContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- SPARK skills: \`spark:skill-name\` (from ~/.config/opencode/spark/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/spark')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/spark && git pull` to update spark.' :
        '';
...
**Below is the full content of your 'spark:using-spark' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
### 1. Install SPARK Skills

```bash
# Clone spark skills to OpenCode config directory
mkdir -p ~/.config/opencode/spark
git clone https://github.com/adityaaria/SPARK.git ~/.config/opencode/spark
```

### 2. Install the Plugin

The plugin is included in the spark repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/spark/.opencode/plugin/spark.js`
...
ln -s ~/.config/opencode/spark/.opencode/plugin/spark.js .opencode/plugin/spark.js
```
...
```
use use_skill tool with skill_name: "spark:brainstorming"
```
...
Personal skills override spark skills with the same name.

## Updating

```bash
cd ~/.config/opencode/spark
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/spark/.opencode/plugin/spark.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/spark/skills`
...
- Documentation: https://github.com/adityaaria/SPARK
```
...
SPARK works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/spark-codex use-skill spark:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/spark-codex bootstrap | head -30`
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/spark.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/spark-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/spark-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/spark.js` created with tools and hooks
