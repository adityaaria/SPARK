...
Add full spark support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port spark to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/spark-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/spark.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/spark/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/spark-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "spark:brainstorming")')
  }),
...
1. **Inject using-spark content**
   - Full content of the using-spark skill
...
export const SPARKPlugin = async ({ client, directory, $ }) => {
  const sparkDir = path.join(process.env.HOME, '.config/opencode/spark');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingSPARK = await readSkill('using-spark');
...
├── .codex/
│   ├── spark-codex        # UPDATED: Use skills-core
│   ├── spark-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── spark.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/spark-codex`
...
2. Update `.codex/spark-codex` to use shared core
...
1. Create `.opencode/plugin/spark.js`
