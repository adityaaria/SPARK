...
**Goal:** Add first-class Pi package support for SPARK and add Pi as a Drill eval backend.

**Architecture:** The Pi package is declared in the root `package.json` and loads existing `skills/` plus a small Pi extension. The extension injects the `using-spark` bootstrap into provider context as a user-role message on session startup and after compaction, with Pi-specific tool mapping. Drill gains a `pi` backend, Pi session-log normalization, and tests.
...
Create `tests/pi/test-pi-extension.mjs` with tests that import `extensions/spark.ts`, register fake Pi handlers, and assert:
- root `package.json` has `keywords` containing `pi-package`
- root `package.json` has `pi.skills: ["./skills"]`
- root `package.json` has `pi.extensions: ["./extensions/spark.ts"]`
...
Expected: FAIL because `extensions/spark.ts` does not exist and `package.json` lacks the `pi` manifest.
...
**Files:**
- Create: `skills/using-spark/references/pi-tools.md`
- Modify: `tests/pi/test-pi-extension.mjs`

- [ ] **Step 1: Write failing test for Pi reference doc**

Add assertions that `skills/using-spark/references/pi-tools.md` exists and documents mappings for `Skill`, `Task`, `TodoWrite`, and built-in tool names.
...
Create `skills/using-spark/references/pi-tools.md` explaining Pi-native skills, optional `pi-subagents`, no canonical todo/tasklist plugin, and built-in lowercase tools.
...
- Pi backend command starts with `pi` and includes `-e ${SPARK_ROOT}`
...
Configure the backend to run `pi -e ${SPARK_ROOT}`, use permissive TUI readiness, `/quit` shutdown, and Pi session log location.
