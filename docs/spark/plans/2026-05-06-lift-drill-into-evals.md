# Lift drill into spark as `evals/` — implementation plan
...
**Goal:** Move the standalone `obra/drill` skill-compliance benchmark into spark as a top-level `evals/` directory, delete redundant bash tests under `spark/tests/` after per-file subagent verification of drill scenario coverage, and update top-level docs so contributors land on the new structure.

**Architecture:** Single PR against `dev` on a new branch `f/evals-lift`. Drill source is copied verbatim with explicit rsync excludes to keep `.git/`, `.venv/`, etc. out of the new dir. A small helper in `drill/cli.py` defaults `SPARK_ROOT` to the parent of the `evals/` directory, so contributors don't have to set the env var. Each bash-test deletion is gated by a subagent that compares the bash test's assertions to its claimed drill scenario's verify block. Historical references in plan docs and release notes are annotated, not rewritten.

**Tech Stack:** Python 3.11 + uv (drill's existing toolchain, unchanged); rsync; bash; git.

**Spec:** `docs/spark/specs/2026-05-06-lift-drill-into-evals-design.md` — read this first.

**Drill source location:** `/Users/jesse/Documents/GitHub/spark/drill/` (sibling to `spark/`).
...
```bash
cd /Users/jesse/Documents/GitHub/spark/spark
git status --short
```
...
```bash
cd /Users/jesse/Documents/GitHub/spark/drill
...
```bash
cd /Users/jesse/Documents/GitHub/spark/spark
test -d /Users/jesse/Documents/GitHub/spark/drill && echo "drill source: OK"
...
rsync -a \
  ...
  --exclude='*.pyc' \
  /Users/jesse/Documents/GitHub/spark/drill/ \
  evals/
```
...
rsync of obra/drill@$DRILL_SHA into spark/evals/, excluding
...
while read -r f; do
  ...
  sha2=$(shasum -a 256 "/Users/jesse/Documents/GitHub/spark/spark/evals/${f#./}" | cut -d' ' -f1)
  if [ "$sha1" != "$sha2" ]; then
...
```bash
cd /Users/jesse/Documents/GitHub/spark/spark/evals
uv sync
```
...
Expected: starts with scenario names. (Will likely error or warn about missing SPARK_ROOT — that's fine, fixed in next task.)
...
/Users/jesse/Documents/GitHub/spark/drill into
/Users/jesse/Documents/GitHub/spark/spark/evals.

Verify:

1. The lift commit message records the SHA reported by:
  cd /Users/jesse/Documents/GitHub/spark/drill && git rev-parse HEAD
...
## Task 5: Add `SPARK_ROOT` default helper
...
```bash
sed -n '1,20p' /Users/jesse/Documents/GitHub/spark/spark/evals/drill/cli.py
```
...
```python
def test_set_spark_root_default_when_unset(monkeypatch, tmp_path):
    """When SPARK_ROOT is unset, helper sets it to PROJECT_ROOT.parent."""
    monkeypatch.delenv("SPARK_ROOT", raising=False)
    from drill.cli import _set_spark_root_default, PROJECT_ROOT

    _set_spark_root_default()

    import os
    assert os.environ["SPARK_ROOT"] == str(PROJECT_ROOT.parent)


def test_set_spark_root_default_respects_existing(monkeypatch):
    """When SPARK_ROOT is already set, helper does not override."""
    monkeypatch.setenv("SPARK_ROOT", "/custom/path")
    from drill.cli import _set_spark_root_default

    _set_spark_root_default()

    import os
    assert os.environ["SPARK_ROOT"] == "/custom/path"
```
...
```bash
cd /Users/jesse/Documents/GitHub/spark/spark/evals
uv run pytest tests/test_cli.py -k set_spark_root_default -v
```

Expected: 2 tests fail with `AttributeError: module 'drill.cli' has no attribute '_set_spark_root_default'`.

- [ ] **Step 4: Add the helper to cli.py**

Edit `/Users/jesse/Documents/GitHub/spark/spark/evals/drill/cli.py`. Replace lines 1–14 with:
...
def _set_spark_root_default() -> None:
    """Default SPARK_ROOT to the parent of evals/ if not already set.

    Drill historically required contributors to export SPARK_ROOT
    pointing at the spark checkout. After lifting drill into
    spark/evals/, the parent of PROJECT_ROOT is always the
    spark root, so we can supply this default automatically.

    Existing SPARK_ROOT environment values are respected as overrides.
    """
    os.environ.setdefault("SPARK_ROOT", str(PROJECT_ROOT.parent))


_set_spark_root_default()
```

The bottom-of-module call to `_set_spark_root_default()` runs at import time, immediately after `load_dotenv()`. This ensures both `engine.py` and `setup.py` (which read `os.environ["SPARK_ROOT"]` directly) and the YAML interpolation (which reads `os.environ` when the backend YAML is loaded) all see the value.
...
git commit -m "evals: default SPARK_ROOT to parent of evals/ if unset

Adds _set_spark_root_default() to drill/cli.py, called at
module import after load_dotenv(). PROJECT_ROOT resolves to evals/
post-lift; its parent is the spark repo root, which is the
correct value for SPARK_ROOT.
...
**Files:**
- Modify: `evals/backends/codex.yaml` (drop `SPARK_ROOT` from `required_env`)
- Modify: `evals/backends/gemini.yaml` (drop `SPARK_ROOT` from `required_env`)

The five `claude*.yaml` backend configs interpolate `${SPARK_ROOT}` into `args` for the `--plugin-dir` flag — they keep `SPARK_ROOT` in `required_env` because the interpolation needs it. The codex/gemini configs only listed it for engine.py/setup.py's `os.environ` reads, which the helper now satisfies.

- [ ] **Step 1: Confirm current state**

```bash
grep -A3 'required_env:' /Users/jesse/Documents/GitHub/spark/spark/evals/backends/codex.yaml
grep -A2 'required_env:' /Users/jesse/Documents/GitHub/spark/spark/evals/backends/gemini.yaml
```

Expected outputs include `- SPARK_ROOT` lines.

- [ ] **Step 2: Read codex.yaml fully**

```bash
cat /Users/jesse/Documents/GitHub/spark/spark/evals/backends/codex.yaml
```

- [ ] **Step 3: Edit codex.yaml — drop the `- SPARK_ROOT` line under `required_env`**
...
- [ ] **Step 4: Edit gemini.yaml — drop the `- SPARK_ROOT` line under `required_env`**
...
git commit -m "evals: drop SPARK_ROOT from codex/gemini required_env

These backends only read SPARK_ROOT via engine.py/setup.py's
os.environ access, which the new cli.py default helper supplies
automatically. claude*.yaml keep SPARK_ROOT in required_env
because they interpolate \${SPARK_ROOT} into --plugin-dir args."
```
...
For tests that assert `SPARK_ROOT` membership in `codex.yaml`'s or `gemini.yaml`'s `required_env`: invert the assertion to confirm absence. Example:

```python
# Before:
def test_codex_requires_spark_root():
    backend = load_backend("codex")
    assert "SPARK_ROOT" in backend.required_env

# After:
def test_codex_does_not_require_spark_root():
    """codex.yaml dropped SPARK_ROOT from required_env;
    the cli.py helper supplies the default."""
    backend = load_backend("codex")
    assert "SPARK_ROOT" not in backend.required_env
```
...
**Files:**
- Modify: `evals/README.md` (drop SPARK_ROOT setup step)
- Modify: `evals/CLAUDE.md` (drop SPARK_ROOT setup step)
...
```bash
export SPARK_ROOT=/path/to/spark
...
`SPARK_ROOT` defaults to the parent of `evals/` (the spark repo root) and only needs to be set if you're running drill against a different spark checkout.
```
...
`SPARK_ROOT` defaults to the parent of `evals/` (the spark repo root). Override only if running drill against a different spark checkout.
```
...
git commit -m "evals: drop SPARK_ROOT setup step from README/CLAUDE
...
Expected: scenario list, no error about missing SPARK_ROOT.
...
If FAIL, debug before continuing. The path-defaults change is the most likely culprit; check that the helper actually fired by adding a `print(os.environ["SPARK_ROOT"])` after the helper call temporarily.
...
```bash
ls /Users/jesse/Documents/GitHub/spark/spark/tests/skill-triggering/prompts/
```
...
```bash
for f in /Users/jesse/Documents/GitHub/spark/spark/tests/explicit-skill-requests/*.sh /Users/jesse/Documents/GitHub/spark/spark/tests/explicit-skill-requests/prompts/*.txt; do
  echo "=== $f ==="
  cat "$f" | head -30
done
```
...
```bash
ls /Users/jesse/Documents/GitHub/spark/spark/evals/fixtures/sdd-go-fractals/
ls /Users/jesse/Documents/GitHub/spark/spark/evals/fixtures/sdd-svelte-todo/
```
...
```bash
cat /Users/jesse/Documents/GitHub/spark/spark/tests/claude-code/test-subagent-driven-development.sh
```
...
- Annotate (do not rewrite): `RELEASE-NOTES.md`, `docs/spark/plans/*.md`
...
| `docs/spark/plans/*.md` | Annotate, don't rewrite (dated artifact) |
...
For each `RELEASE-NOTES.md` or `docs/spark/plans/*.md` hit, add an inline annotation at the *first* hit per file:
...
```
Working directory: /Users/jesse/Documents/GitHub/spark/spark
...
Search the entire spark tree (excluding evals/, node_modules/,
...
and docs/spark/plans/*.md are annotated as dated artifacts,
not rewritten."
```
...
Open `/Users/jesse/Documents/GitHub/spark/spark/docs/testing.md` and replace the file content with this structure (preserve the existing Plugin-test details where applicable):

```markdown
# Testing SPARK

SPARK has two distinct kinds of tests, each in its own directory:
...
Open `/Users/jesse/Documents/GitHub/spark/spark/.gitignore` and add at the bottom:
...
```bash
cd /Users/jesse/Documents/GitHub/spark/spark/tests/brainstorm-server
...
**Branch:** f/evals-lift, in /Users/jesse/Documents/GitHub/spark/spark
**Base:** dev (currently b4363df)
**Spec:** docs/spark/specs/2026-05-06-lift-drill-into-evals-design.md

This branch lifts the obra/drill repo into spark/evals/ and
...
3. Does the SPARK_ROOT helper actually default correctly when
   the env var is unset? (cd evals && unset SPARK_ROOT && uv
...
gh pr create \
  ...
  --head f/evals-lift \
  --reviewer arittr \
  --title "Lift drill into spark as evals/ harness" \
  --body "$(cat <<'EOF'
## What problem are you trying to solve?

Drill — the standalone Python skill-compliance benchmark at obra/drill — is already the de facto eval harness for spark. The PRI-1397 commit series lifted ~22 bash tests into drill scenarios, and the most recent spark commit (a2292c5) explicitly removed a redundant bash test with the message "replaced by drill behavioral coverage". Drill is a sibling repo today, requiring contributors to clone two checkouts and set SPARK_ROOT manually. This PR completes the migration: drill becomes spark/evals/.

## What does this PR change?

- Lifts the obra/drill repo into spark as `evals/`, with explicit rsync excludes (.git, .venv, results, .env, __pycache__, *.egg-info, .private-journal). The lift commit records the source SHA.
- Adds a `_set_spark_root_default()` helper to drill/cli.py so SPARK_ROOT defaults to the parent of evals/ — no manual env-var setup.
- Drops SPARK_ROOT from required_env in codex.yaml/gemini.yaml (the helper supplies it). Claude*.yaml keep it because they interpolate ${SPARK_ROOT} into --plugin-dir args.
...
Yes. Cross-runtime evaluation is core to spark, the migration to drill scenarios was already underway in this repo, and the eval harness needs to be discoverable in-tree to be findable.

## What alternatives did you consider?

- Vendored copy + sync script (drill repo continues independently). Rejected: divergence risk; single-source-of-truth wins.
- git subtree merge (preserves drill history in-tree). Rejected: spark' git history grows by 50+ commits, the merge commit is ugly, subtrees are operationally heavy.
- Keep drill as a sibling repo and just polish docs. Rejected: doesn't solve the discoverability problem.

## Does this PR contain multiple unrelated changes?

No — every change supports "drill is now evals/ inside spark". Multiple commits for atomicity (verbatim copy, env helper, YAML updates, docs) but one direction.

## Existing PRs

- [x] I have reviewed all open AND closed PRs for duplicates or prior art
- Related PRs: #1486 (adityaaria/SPARK cross-platform PR — independent; no shared file changes besides README, which has no overlap)
...
- Initial prompt: see linked spec (`docs/spark/specs/2026-05-06-lift-drill-into-evals-design.md`).
- Drill's own pytest suite passes.
- One drill scenario re-run from the new location end-to-end (proves the SPARK_ROOT default works).
...
1. Archive obra/drill on GitHub (mark read-only, add README pointer to adityaaria/SPARK/evals/).
...
- [ ] `cd evals && unset SPARK_ROOT && uv run pytest` passes
- [ ] `cd evals && unset SPARK_ROOT && uv run drill list` returns scenarios
- [ ] `cd evals && unset SPARK_ROOT && uv run drill run triggering-test-driven-development -b claude` passes
- [ ] `tests/brainstorm-server/server.test.js` still passes (regression gate for non-LLM tests)
- [ ] `git diff dev..HEAD docs/spark/plans/2026-04-06-worktree-rototill.md docs/spark/plans/2026-03-23-codex-app-compatibility.md RELEASE-NOTES.md` shows annotations only, no path rewrites
