...
**Spec:** `docs/spark/specs/2026-06-09-sdd-task-scoped-review-dispatch-design.md` — read it before starting. Decisions already settled there: full re-reviews stay; the two review stages stay separate; coordinator keeps model judgment; `requesting-code-review/` stays broad.
...
```
- **spark:requesting-code-review** - Code review template for reviewer subagents
```
...
Lives in the `evals/` **submodule** (separate repo, `spark-evals`). Work on a branch there; the parent submodule-pointer bump happens at finishing time per `evals/CLAUDE.md`.
...
def scaffold_sdd_quality_defect_plan(workdir: Path) -> None:
    ...
    plans_dir = workdir / "docs" / "spark" / "plans"
...
You have a small plan at docs/spark/plans/report-plan.md — two report
...
"I have a small plan at docs/spark/plans/report-plan.md — two report
formatting functions. Use the spark:subagent-driven-development skill
...
- A `Skill` invocation naming `spark:subagent-driven-development`
...
pre() {
    ...
    git-branch main
    requires-tool npm
    file-exists 'docs/spark/plans/report-plan.md'
    file-contains 'docs/spark/plans/report-plan.md' 'formatAdminReport'
    file-contains 'docs/spark/plans/report-plan.md' 'repeat\(40\)'
}

post() {
    skill-called spark:subagent-driven-development
    tool-called Agent
...
Run: `grep -n "code-quality-reviewer" skills/using-spark/references/antigravity-tools.md skills/using-spark/references/gemini-tools.md`
...
- [ ] **Step 1: Baseline (skills as released on dev)** — from the main checkout (`/Users/jesse/git/spark/spark`, on dev), or any checkout without this branch's changes:

```bash
cd evals
export SPARK_ROOT=/Users/jesse/git/spark/spark
...
- [ ] **Step 2: After (this branch's skills)** — point `SPARK_ROOT` at this worktree:

```bash
cd evals
export SPARK_ROOT=/Users/jesse/git/spark/spark/.claude/worktrees/sdd-review-dispatch
uv run quorum run scenarios/sdd-rejects-extra-features --coding-agent claude
uv run quorum run scenarios/sdd-go-fractals --coding-agent claude
uv run quorum run scenarios/sdd-svelte-todo --coding-agent claude
uv run quorum run scenarios/spec-reviewer-catches-planted-flaws --coding-agent claude
...
After all tasks pass: the evals submodule commit needs to land in `spark-evals` (PR to its `main`), then this branch bumps the `evals` submodule pointer — per `evals/CLAUDE.md`, the parent bump is part of propagation, not optional. Then use spark:finishing-a-development-branch. PRs against spark target `dev`.
